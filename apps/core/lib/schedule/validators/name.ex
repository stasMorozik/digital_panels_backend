defmodule Core.Schedule.Validators.Name do
  @moduledoc """
    Валидирует название расписания
  """
  ...
  def valid(name) when is_binary(name) do
    with true <- String.match?(name, @format),
      ...
      false -> {:error, "Невалидное название расписания"}
    end
  end

  def valid(_) do
    {:error, "Невалидное название расписания"}
  end
end

defmodule Task.UpdatingTest do
  use ExUnit.Case

  alias PostgresqlAdapters.Task.Inserting
  alias PostgresqlAdapters.Task.Updating
  alias PostgresqlAdapters.Task.GettingById
  alias Core.Task.Builder

  doctest PostgresqlAdapters.Task.Updating
  ...
  test "Update 0" do
    ...
    {result, _} = Updating.transform(task, user)
  ...
  test "Update 1" do
    ...
    {:ok, task} = Core.Task.Editor.edit(task, %{
      name: "Test",
      playlist: playlist,
      group: group,
      type: "Каждый день",
    ...
    {:ok, true} = Updating.transform(task, user)

    {:ok, task} = GettingById.get(task.id, user)

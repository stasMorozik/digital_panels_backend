defmodule Core.Schedule.Builders.Sum do
  ...
  def build({:ok, entity}, {start_hour, start_minute}, {end_hour, end_minute}) do
    with {:ok, true} <- Core.Schedule.Validators.Hour.valid(start_hour),
         ...
         {:ok, true} <- Core.Schedule.Validators.Hour.valid(end_hour),
         {:ok, true} <- Core.Schedule.Validators.Minute.valid(end_minute),
         sum_end <- (end_hour * 60) + end_minute,
         {:ok, true} <- Core.Schedule.Validators.Sum.valid(sum_start, sum_end) do
